...
```{r Load_the_Data}
# Load the Data
setwd('~/Desktop/DAND_P4/p4_US_campaign_funding')
...
Constructing a frequency plot makes me see something about _Sanders, Bernard_ (who has a big dot low down). Some candidates have a small distribution, and some a wider one, there is another person with a big dot at the bottom, yet also a quite big one higher up (_Clinton, Hillary_); and further one person with a very high distribution up and down, but also a strong base it seems (_Cruz, Ted_). Also _Carson, Benjamin_ shows a large number of contributions in the lower amounts.

```{r Exploring_Most_Money_1_x_add_on}

# thinking about binning data and about displaying statistical values in graphs
# thanks: https://discussions.udacity.com/t/plotting-statistical-overlays/167266/2

df.mean <- aggregate(df['contb_receipt_amt'], df['cand_nm'], mean)
df.median <- aggregate(df['contb_receipt_amt'], df['cand_nm'], median)

# median line plot on mean bar chart
ggplot(df.mean, aes(cand_nm, contb_receipt_amt)) +
  geom_bar(stat = 'identity', fill='lightblue') +
  ggtitle('median point plot on mean bar chart') +
  labs(x = 'candidate',
       y = '(mean) contribution $') +
  theme(axis.text.x = element_text(angle = 90)) +
  geom_point(data = df.median,
             aes(x=cand_nm, y=contb_receipt_amt, group=1),
             size=1, color="orange")

# mean line plot on median bar chart  
ggplot(df.median, aes(cand_nm, contb_receipt_amt)) +
  geom_bar(stat = 'identity', fill='orange') +
  ggtitle('mean point plot on median bar chart') +
  labs(x = 'candidate',
...
```{r fig.height=16, Plot_Two}
# adapting the global fig.height knitr-property to make the graph more readable
...
ggplot(only_contribs, aes(contb_receipt_amt, contbr_city)) +
  ...
  ggtitle('Individual Contributions to US Presidential elections in CA cities') +
  theme(plot.title = element_text(size = 25)) +
  labs(x = 'Amount in USD',
       y = 'All CA Cities (those with 30 000+ contributions are labeled)')
...
While most of these cities are also among the 5 most populous cities in CA, **Oakland** holds only the 8th place in [that ranking](https://en.wikipedia.org/wiki/List_of_largest_California_cities_by_population).
...
- https://discussions.udacity.com/t/plotting-statistical-overlays/167266/2
