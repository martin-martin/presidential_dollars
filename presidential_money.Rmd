...
```{r echo=FALSE, Exploring_Most_Money_1_1}
...
```{r echo=FALSE, Exploring_Most_Money_1_2}
...
```{r echo=FALSE, Exploring_Most_Money_1_3}
########################## TAKE THIS ONE ##########################
...
# IDEA: plot mean amount of contribution vs. number of contributions
```
...
```{r echo=FALSE, message=FALSE, Exploring_Most_Money_2_1}
...
```{r echo=FALSE, Exploring_Most_Money_2_2}
...
```{r echo=FALSE, Exploring_Most_Money_2_3}
nrow(only_refunds)
```

So I tried a plot for these refunds (which was too big, so I'll take a look at the most Dollars)

```{r echo=FALSE, Exploring_Most_Money_2_4}

# a plot containing the highest refunded people
ggplot(subset(refunds_by_contb, contb_receipt_amt < -10000), aes(contb_receipt_amt, contbr_nm)) +
  geom_point()
```
...
```{r echo=FALSE, Exploring_Most_Money_2_5}

only_contribs <- subset(df, contb_receipt_amt > 0)
ggplot(subset(only_contribs, contb_receipt_amt > 10000), aes(contb_receipt_amt, contbr_nm)) +
  geom_point()
...
```{r echo=FALSE, Exploring_Most_Money_2_6}
...
```{r echo=FALSE, Exploring_Most_Money_2_7}
...
```{r echo=FALSE, Exploring_Most_Money_3_1}
...
```{r echo=FALSE, Exploring_Most_Money_3_2}
...
```{r echo=FALSE, Exploring_Most_Money_3_3}
...
```{r echo=FALSE, Exploring_Most_Money_3_4}
...
```{r echo=FALSE, Exploring_Most_Money_3_5}
...
```{r echo=FALSE, Exploring_Most_Money_3_6}
...
```{r echo=FALSE, Exploring_Most_Money_3_7}
...
```{r echo=FALSE, Exploring_Most_Money_3_8}
...
```{r echo=FALSE, Exploring_Most_Money_3_9}
...
```{r echo=FALSE, Exploring_Most_Money_4_1}
...
```{r echo=FALSE, message=FALSE, Exploring_Most_Money_5_1}
...
```{r echo=FALSE, message=FALSE, Exploring_Most_Money_5_3}
...
```{r echo=FALSE, Exploring_Cities_1}
...
```{r echo=FALSE, Exploring_Cities_2}
ggplot(df, aes(contb_receipt_amt, contbr_city)) + scale_y_discrete() +
  geom_count()
...
```{r echo=FALSE, Exploring_Cities_3}
...
ggplot(only_contribs, aes(contb_receipt_amt, contbr_city)) + scale_y_discrete() +
  ...
  # removing the ticks and text of the y axis, because there are too many to show
...
```{r echo=FALSE, Exploring_Cities_4}
...
```{r echo=FALSE, Exploring_Cities_5}
...
```{r echo=FALSE, Exploring_Cities_6}
...
```{r echo=FALSE, Exploring_Cities_7}
...
```{r echo=FALSE, warning=FALSE, Exploring_Cities_8}
# I set the 'warning=FALSE' option here because it otherwise knits a long streak of
# "## Warning in FUN(X[[i]], ...): no non-missing arguments to min; returning Inf"
# (I think) this Warning comes because not all i have duplicates
...
total_contribs <- total_contribs[with(total_contribs, ave(POPESTIMATE2012, contbr_city, FUN=min) == POPESTIMATE2012), ]
head(total_contribs)

# IDEA: plot the relations in a meaningful graph (or graphs!)
...
```{r echo=FALSE, warning=FALSE, Exploring_Cities_9}
...
```{r echo=FALSE, warning=FALSE, Exploring_Cities_10}
...
```{r echo=FALSE, warning=FALSE, Exploring_Cities_11}

# creating a df that contains only contributions made in the 5 cities with more than 30 000 contributions
most_contribs <- subset(only_contribs, only_contribs$contbr_city %in% five_cities)

# calculating the percentage of contributions made in the 5 most-contribution-cities
...
```{r echo=FALSE, Exploring_Unemployment_1}
...
```{r echo=FALSE, Exploring_Unemployment_2}
...
```{r echo=FALSE, Exploring_Unemployment_3}
...
```{r echo=FALSE, warning=FALSE, Exploring_Unemployment_4}
...
```{r echo=FALSE, warning=FALSE, Exploring_Unemployment_5}
...
```{r echo=FALSE, Exploring_Unemployment_6}
...
```{r echo=FALSE, Exploring_Unemployment_7}
...
#cand_id_map
still_cand = c('P60007168', 'P60006723', 'P60005915', 'P00003392', 'P60003670', 'P80001571', 'P60006111')
...
c_party_cand_money <- aggregate(contb_receipt_amt ~ cand_party + cand_nm, data = current_cands, sum)
# rearrange factors for visualization
c_party_cand_money$cand_nm_ord <- factor(c_party_cand_money$cand_nm, levels=c("Sanders, Bernard", "Clinton, Hillary Rodham", "Trump, Donald J.", "Rubio, Marco", "Kasich, John R.", "Cruz, Rafael Edward 'Ted'", "Carson, Benjamin S."))
...
# plot the graph
ggplot(c_party_cand_money, aes(cand_party, contb_receipt_amt, fill=cand_nm_ord)) +
...
ggplot(only_contribs, aes(contb_receipt_amt, contbr_city)) +
  geom_jitter(alpha=0.1) + 
  # limiting the graph to the cities with the most contributions
  scale_y_discrete(limits=c(five_cities)) +
  ggtitle('Individual Contributions to the US Presidential elections\nin the 5 CA cities with the highest number of Contributions') +
  labs(x = 'Amount in USD', y = 'Cities with more than 30 000 contributions')
...
ggplot(current_cands, aes(x = contb_emp_status, y = cand_nm)) +
  ...
  scale_size_area(name="Scale of\nproportions\n(of number of\ncontributions\nreceived)", max_size = 20) +
